...
if __name__ == '__main__':
    ...
    for log_file_name in log_files:
        with open(log_file_name) as log_file:
            ...
            for log_data in log_data_list:
                ...
                time = log_data['throughput']
                if not size_list or size in size_list:
                    if name in data_list:
                        data_list[name].append(time)
                    else:
                        data_list[name] = [time]

    throughput_list = {}
    for name in data_list:
        throughput = sum(data_list[name]) / len(data_list[name])
        throughput_list[name] = throughput
        if throughput > max_time :
            max_time = throughput
    ...
    with open(data_file, mode='w') as f:
        ...
        for name in name_list:
            if name in throughput_list :
                time_list.append(str(throughput_list[name]))
            else:
                time_list.append('-')
    ...
    with open(args.plot_script, mode='w') as f:
        ...
        print('set ylabel "{}"'.format('throughput [mwords/sec]'), file=f)
